...
Bit vectors may also be transformed into (decimal) integers, e.g. the binary number 101 equals the decimal number 5.
...
\begin{snugshade}
\begin{lstlisting}
clado_prob := fnCladoProbs(clado_type_prob, n_areas, 2)
\end{lstlisting}
\end{snugshade}
...
mn[mni++] = mnFile(clock_bg, d[1][2], d[1][3], d[2][1], d[2][3], d[3][1], d[3][2], e[1], e[2], e[3], widespread_sympatry, subset_sympatry, allopatry, file=out_fn+".params.txt")
\end{lstlisting}
\end{snugshade}

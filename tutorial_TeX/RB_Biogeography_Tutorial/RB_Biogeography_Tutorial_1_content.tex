...
Exposing the structure of the DEC rate matrix is useful when learning about the model, but typically the matrix may be constructed with the following convenience function, provided {\tt d} and {\tt e} are defined.

\begin{snugshade}
\begin{lstlisting}
er_nat := fnStructureDispersalExtinction(d, e)
er := simplex(er_nat)
bf <- simplex(rep(1,n_states))
q := fnFreeK(er,bf)
\end{lstlisting}

...
The epoch analysis (Fig \ref{fig:epoch_phy}) produces more sensible ancestral range estimates, with Kauai being colonized first, and younger islands only being colonized as they become available.
...
Treating $P=0.95$ as significant support for an evolutionary outcome, the epoch model can assert that crown silverswords originated after the formation of Kauai, $P(a_s > a_K) = 0.02 < 1-0.95$, but can only provide weak support that they originated after the formation of Oahu, $P(a_s > a_O) = 0.26 > 1-0.95$.

\section{Modelling incomplete lineage sorting: the multispecies coalescent}
...
Thus, our prior is that each population size per branch is identically distributed from an exponential distribution with rate 1.
...
for (i in 1:n_branches) {
  ...
  moves[++mi] = mvScale(Ne[i],1,tune=true,weight=1.0)
}
...
We will use the common \code{PhyloCTMC} distribution, which is a continuous time Markov process along a phylogenetic tree.
We create a \code{seq} variable and attach/clamp each gene to one of the \code{seq} variables.
{\tt \begin{snugshade*}
\begin{lstlisting}
for ( i in 1:num_loci ) { 

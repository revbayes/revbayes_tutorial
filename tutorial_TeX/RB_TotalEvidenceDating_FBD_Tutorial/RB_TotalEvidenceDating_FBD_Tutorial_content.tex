...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=0.7\textwidth,angle=0]{\ResourcePath figures/tree_plot_with_fossils}}
\caption{\small  }
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=\textwidth,angle=0]{\ResourcePath figures/tikz/fbd_gm}}
...
\label{fig:morpho_gm}
\end{figure}
All of the morphological characters provided in the data matrix for this tutorial are binary.  
...
\subsection{The Data}\label{subsect:RB-DataFiles}
...
\subsubsection{The FBD Distribution Object}\label{subsub:RB-FBD-dnFBD}
...
Specifically, we assume the likelihood of a particular fossil observation is equal to one if the fossil's inferred age on the tree falls within its observed stratigraphic range, and zero otherwise.
We can represent this in \RevBayes using any uniform distribution that is proportional to the likelihood, i.e. non-zero when the likelihood is equal to one.

For example, the likelihood is equal to one whenever $T_{\min} < t_i < T_{\max} \implies t_i - T_{\max} < 0 < t_i - T_{\min}$, where $t_i$ is the inferred fossil age, and $(T_{\min},T_{\max})$ is the stratigraphic range.
So let's represent the likelihood using a random variable uniformly distributed in $(t_i - T_{\max}, t_i - T_{\min})$ and clamped at zero.
...
for(i in 1:fossil_intervals.size())
{
    ...
    T_min = fossil_intervals[i][2]
    T_max = fossil_intervals[i][3]
    
    t[i] := tmrca(fbd_tree, clade(taxon))
        
    fossil[i] ~ dnUniform(t[i] - T_max, t[i] - T_min)
    fossil[i].clamp( 0 )
}
\end{lstlisting}
\end{snugshade*}}
...
As with the diversification and turnover nodes specified in Section \ref{subsub:RB-FBD-SpeciationExtinction}, we can create deterministic nodes to sample the posterior distributions of these parameters. 
...
\bigskip
\subsection{Execute the MCMC Analysis}\label{subsect:RB-RunMCMC}

...
This tutorial demonstrates how to specify the models used in a time-calibrated ``total-evidence'' phylogenetic analysis of extant and fossil species, combining morphological and molecular data as well as stratigraphic range data from the fossil record.
...
One example of such an uncorrelated relaxed model is the ``Uncorrelated exponential relaxed clock'' (or UExp), in which the substitution rate for each lineage is assumed to be independent and identically distributed according to an exponential density (Fig. \ref{fig:uexp_gm}).
...
In the tutorial, we will relax this assumption by allowing the stationary 1-state probability to take on any value between 0 and 1, drawn from a beta prior distribution.
...
Then, similar to the way we model site-specific substitution rates using a discretized gamma distribution (described in \href{https://github.com/revbayes/revbayes_tutorial/raw/master/tutorial_TeX/RB_CTMC_Tutorial/RB_CTMC_Tutorial.pdf}{RB\_CTMC\_Tutorial}), we will model site-specific stationary frequencies using a discretized beta distribution \citep{Wright2016}.
...
\RevBayes has two ways of doing this: one is used for datasets in which only parsimony informative characters are observed; the other is for datasets in which parsimony informative characters and non-parsimony informative variable characters (such as autapomorphies) are observed.
\begin{figure}[h!]
\fbox{%
\begin{minipage}{\textwidth}\centering
...
0 & \text{otherwise}
\end{cases}$$
In other words, we assume the likelihood is equal to one if the inferred age $t_i$ is consistent with the fossil record.
...
\section{Estimating the Phylogeny and Divergence Times of Fossil and Extant Bears}\label{sect:Exercise}
...
For the fossil species, this occurrence times are obtained from the literature or fossil databases like the \href{http://fossilworks.org/}{Fossilworks PaleoDB} or the \href{http://fossilcalibrations.org/}{Fossil Calibration Database}, or from your own paleontological expertise. 
...
Next we need to account for uncertainty in the age estimates of our fossils using the stratigraphic range data provided in the file \cl{bears\_fossil\_intervals.tsv}.
First, we read this file into a matrix called \cl{fossil\_intervals}. %, then we loop over this matrix and for each fossil create a uniform random variable.
...
Next, we will loop over this matrix and for each fossil create a uniform random variable.
Remember, we can represent the fossil likelihood using any uniform distribution that is proportional to the likelihood, \IE non-zero when the likelihood is equal to one (section \ref{subsect:Intro-TipSampling}).

For example, we know the likelihood is equal to one whenever $a_i < t_i < b_i$, which implies $t_i - b_i < 0 < t_i - a_i$, where $t_i$ is the inferred fossil age, and $(a_i,b_i)$ is the stratigraphic range.
So let's represent the likelihood using a random variable uniformly distributed in $(t_i - b_i, t_i - a_i)$ and clamped at zero.
...
for(i in 1:fossil_intervals.size())
{
    ...
    a_i = fossil_intervals[i][2]
    b_i = fossil_intervals[i][3]
...
you will see the list of \cl{Trace Files} on the left-hand side.
The bottom left section, called \cl{Traces}, provides a list of every parameter in the log file, along with the mean and the Effective Sample Size (ESS) for the posterior sample of that parameter.
...
The inspection window for your selected parameter is the \cl{Estimates} window, which shows a histogram and summary statistics of the values sampled by the Markov chain. 
Figure \ref{fig:tracer} shows the marginal distribution of the \cl{Posterior} statistic for the analysis provided in the \cl{output} directory.
...
\begin{figure}[h!]
\centering
\fbox{\includegraphics[width=\textwidth,angle=0]{\ResourcePath figures/trace}}
...
You can read more about MCMC tuning and improving mixing in the tutorial \href{https://github.com/revbayes/revbayes_tutorial/raw/master/tutorial_TeX/RB_MCMC_Intro_Tutorial/RB_MCMC_Intro_Tutorial.pdf}{RB\_MCMC\_Intro\_Tutorial} and \href{https://github.com/revbayes/revbayes_tutorial/raw/master/tutorial_TeX/RB_MCMC_Tutorial/RB_MCMC_Tutorial.pdf}{RB\_MCMC\_Tutorial}.

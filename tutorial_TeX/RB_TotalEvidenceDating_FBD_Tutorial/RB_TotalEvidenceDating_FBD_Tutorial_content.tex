...
In this tutorial, our molecular data are best analyzed with a General Time-Reversible model of sequence evolution and gamma-distributed rate heterogeneity. This model of sequence evolution is covered thoroughly in CTMC\_Tutorial. 
...
We create stochastic nodes for these variables, each from a uniform Dirichlet prior distribution.
...
Special moves are required to propose changes to a Dirichlet random variable, also known as a simplex (a vector constrained sum to one). Here, we use a \cl{SimplexElementScale} move, which scales a single element of a simplex and then renormalizes it to sum to one.
{\tt \begin{snugshade*}
\begin{lstlisting}
...
Then we create a Gamma$(\alpha,\alpha)$ distribution, discretized into 4 rate categories using the \cl{DiscretizeGamma} function.
...
Finally, we can create the \cl{PhyloCTMC} distribution for our sequence data, including the gamma-distributed site rate categories, as well as the branch rates defined as part of our exponential relaxed clock.

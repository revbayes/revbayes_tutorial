...
Another possible uncorrelated relaxed clock model is the uncorrelated lognormal model, described in the \href{https://github.com/revbayes/revbayes_tutorial/raw/master/tutorial_TeX/RB_Dating_Tutorial/RB_Dating_Tutorial.pdf}{\mi{Relaxed Clocks}} tutorial.
\begin{figure}[h!]
\fbox{%
\begin{minipage}{\textwidth}\centering
...
Then, similar to the way we model site-specific substitution rates using a discretized gamma distribution (described in the \href{https://github.com/revbayes/revbayes_tutorial/raw/master/tutorial_TeX/RB_CTMC_Tutorial/RB_CTMC_Tutorial.pdf}{\mi{Substitution Models}} tutorial), we will model site-specific stationary frequencies using a discretized beta distribution \citep{Wright2016}.
\begin{figure}[h!]
\fbox{%
\begin{minipage}{\textwidth}\centering
...
For simplicity, we will assume a strict clock rate---meaning that every branch has the same rate---drawn from an exponential distribution (section \ref{subsub:Intro-MorphClock}).
{\tt \begin{snugshade*}
\begin{lstlisting}
...
Before creating the summary tree, we must first convert the vector of pruned trees to a tree-trace object and specify a burn-in value. 
We do this using the \cl{treeTrace()} function.
The burn-in value is an option of \cl{treeTrace()} that specifies the number of samples you wish to discard. 
By default, \cl{treeTrace()} will remove the first 25\% of the samples, but we may wish to increase the burn-in to 50\%. 
If you view the size of the \cl{trees} vector (use \cl{trees.size()}), you will see that this list of trees has \cl{1001} elements, thus we will set the burn-in to \cl{500}.
{\tt \begin{snugshade*}
\begin{lstlisting}
trace_pruned = treeTrace(trees,burnin=500)
\end{lstlisting}
\end{snugshade*}}
...
{\tt IcyTree} has many unique options for visualizing phylogenetic trees and can produce publication-quality vector files (\IE svg). 
Additionally, it correctly represents sampled ancestors on the tree as a node with only one descendant (Fig.\ \ref{fig:IcyTreeSumm}). 
\begin{figure}[h!]
\fbox{%
\begin{minipage}{\textwidth}\centering
\includegraphics[width=0.85\textwidth,angle=0]{\ResourcePath figures/summary_tree.pdf}
...
\QUEST Try to replicate Fig.\ \ref{fig:IcyTreeSumm}. (Hint: \mi{Style\textrightarrow Mark Singletons})
...
\QUEST Try mousing over a branch (see Fig\ \ref{fig:IcyTreeScreenshort}).

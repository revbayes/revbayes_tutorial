...
In order to print nodes in our graph to output (also called \cl{monitoring}), we need to create deterministic nodes for birth and death. Deterministic nodes are often value transformations between existing stochastic nodes. So we will define diversification and turnover as deterministic nodes.
...
All the components of the FBD process are now defined. The next step is to combine these parameters to define the tree prior as the FBD.
...
Finally, we can also add monitors for other quantities we might be interested in estimating. Below, we will define a monitor that prints the number of fossils that are inferred to be 'sampled ancestors' - lineages that are present in the phylogeny, and have descendents present on the tree. We will also define a monitor that prints the age of the crown group of bears, using the previously-defined extant bear constraint.

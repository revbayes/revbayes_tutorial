...
initialize a vector for storing moves:
\\
\cl{moves = VectorMoves()}
\item
push a scaling move on $\sigma$:
\\
\cl{
moves.append( mvScale(sigma, lambda=2.0, tune=true, weight=3.0) )
\\
%index <- index + 1
}
\item
...
\\
\cl{
moves.append( mvRealNodeValTreeSliding(process=logmass, lambda=10, tune=true, weight=100) )
\\
%index <- index + 1
}
\item
...
\\
\cl{
moves.append( mvRealNodeValTreeTranslation(process=logmass,lambda=1,
\\
tune=true,weight=1) )
\\
%index <- index + 1
...
\\
\cl{
moves.append( mvMultivariateRealNodeValTreeSliding(process=X, lambda=10,
\\
...
\\
\cl{
moves.append( mvMultivariateRealNodeValTreeTranslation(process=X, lambda=1,
\\
tune=true, weight=1) )
\\
%index <- index + 1
...
\\
\cl{
moves.append( mvConjugateInverseWishartBrownian(sigma=Sigma, process=X,
\\
...
\\
\cl{moves.append( mvSlide(copestrend, delta=2.0, tune=true, weight=3.0) )
\\
%index <- index + 1}
\end{itemize}

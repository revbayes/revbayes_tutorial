...
Using \cl{Tracer}, visualize the posterior distribution on the per site rate parameter \cl{perSiteRates}.
\item 
What is the posterior probability that \cl{siteRateDiff} is larger than 0.5 (\IE having a larger rate of evolution for body size)?
\end{itemize}
...
Instead of having a fixed tree as in the previous, we should now define a \cl{random} tree. We use a birth death prior with prior distributions on the \cl{diversification} rate and \cl{turnover} rate.
{\tt \small \begin{snugshade*}
\begin{lstlisting}
...
How does the posterior distribution of \cl{sigma} looks compared with the first and second analysis?
\item
Calculate the 95\% credible set of trees using the \cl{treetrace.summary()} command.
\item

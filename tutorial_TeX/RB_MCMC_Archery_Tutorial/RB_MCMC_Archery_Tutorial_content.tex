...
\includegraphics[width=1.8in,angle=0]{\ResourcePath figures/target.pdf}
\caption{Visual representation of the archery data used in this tutorial.
Each dot represents the position of an arrow shot by an archer.
...
%\impmark 
{\begin{framed}
Make yourself familiar with the example script called \cl{archery\_MH.Rev} which shows the code for the following sections. Then, start a new and empty script in your text editor and follow each step provided  as \colorbox{shadecolor}{\tt{Rev commands}} below.
\end{framed}}
...
We will start this part by defining the number of iterations for our MCMC (\cl{reps = 1000}) and writing the first line of our \cl{for} loop. 
{\tt \begin{snugshade*}
\begin{lstlisting}
reps = 10000 
for(rep in 1:reps){
...
\begin{enumerate}[label=\textnormal{Step \arabic*)}]
	\item Write and execute this script, which you can give any name you like (there is also an example file called \cl{archery\_MH.Rev}).
...
\begin{enumerate}[label=\textnormal{Step \arabic*)}]
	...
	\item How does the ESS change if you use a \cl{delta=10} for the sliding-window move? How about \cl{delta=0.1}?
	\item How does the ESS change if you use \cl{lambda=10} or \cl{lambda=0.1} for the scaling move?
...
\hline
\vspace{1mm}
Sliding-window & \TextField[name=pp11,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{}  & \TextField[name=pp12,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{}  & \TextField[name=pp13,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{}\\
\hline
\vspace{1mm}
Scaling & \TextField[name=pp21,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{}  & \TextField[name=pp22,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{}  & \TextField[name=pp23,backgroundcolor={.85 .85 .85},color={0 0 1},height=4ex]{} \\
\hline
\end{tabular}}
\label{tab:pp}
\end{table}

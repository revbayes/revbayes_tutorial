...
\caption{placeholder figure for a representation of the model}
\end{minipage}}
...
First, let us rewrite the MCMC loop so that instead we call a function, which we name \cl{move\_prior} for simplicity, that performs the move:
{\tt \begin{snugshade*}
\begin{lstlisting}    
for (rep in 1:reps){
...
%TODO: We don't need to specify a type for the argument, so maybe we should just change this to `(weight)` instead of `(Natural weight)` (TAH)
{\tt \begin{snugshade*}
\begin{lstlisting}    
...
% TODO: verify if this separate script is provided. I don't think we need to. (TAH)
...
% TODO: are argument types needed? (TAH)
{\tt \begin{snugshade*}
\begin{lstlisting}    

...
Since the first step in this exercise is to assume a single model for both genes, we need to combine the three datasets.
Concatenate the three data matrices using the \cl{concatenate()} function.
This returns a single data matrix with all three genes.

{\tt \begin{snugshade*}
\begin{lstlisting}
data = concatenate( data_16s, data_cox2, data_cytb )
\end{lstlisting}
\end{snugshade*}}
...
For later use, we will store the number of species (\cl{n\_species}), the tip labels (\cl{names}), and the number of internal branches (\cl{n\_branches}).
